...
class FluxControlNetPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        if isinstance(inner_module, FluxControlNetModel):
        ...
        elif isinstance(inner_module, FluxMultiControlNetModel):
            control_images = []
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                controlnet_block_samples, controlnet_single_block_samples = self.controlnet(
                    hidden_states=latents,                   # (bs,4096,64)
                    controlnet_cond=control_image,           # (bs,4096,64)
                    controlnet_mode=control_mode,            # (bs,1)
                    conditioning_scale=controlnet_conditioning_scale, # float
                    timestep=timestep / 1000,                # (bs,)
                    ...
                    img_ids=latent_image_ids,                # (4096,3)
                    joint_attention_kwargs=self.joint_attention_kwargs,
                    return_dict=False,
                )

                # NOTE: This is important to do in the event that we train with an f32
                # controlnet but have everything else in f16 and this gets called in an
                # eval loop. In that case we need to make sure the dtypes match.
                controlnet_block_samples = [elem.to(dtype=latents.dtype) for elem in \
                    controlnet_block_samples]
                controlnet_single_block_samples = [elem.to(dtype=latents.dtype) for elem in \
